            ...
            $backupToken = 'TOKEN-'.$core->framework->auth->keygen('5').'-'.$core->framework->auth->keygen('10').'-'.$core->framework->auth->keygen('5');
                ...
                if($_POST['backup_params'] != '' && $_POST['backup_params'] != '*') {
                    ...
                    if($core->framework->rcon->online($core->framework->server->getData('server_ip'), $core->framework->server->getData('server_port')) === true){
                        ...
                        ssh2_exec($con, 'cd /srv/scripts; sudo ./send_command.sh '.$core->framework->server->getData('ftp_user').' "save-all"', true);
                        sleep(2);
                        ssh2_exec($con, 'cd /srv/scripts; sudo ./send_command.sh '.$core->framework->server->getData('ftp_user').' "save-off"', true);
                        ...
                        if($newBackup->execute(array(
                            ...
                            ':time' => time()
                                ...
                                if($backup == 'all'){
                                    ...
                                    $s = ssh2_exec($con, 'cd /srv/scripts; sudo ./backup_server.sh '.$core->framework->server->getData('ftp_user').' "'.$filename.' '.$core->framework->server->nodeData('node').' '.$core->framework->server->getData('hash').' '.$backupToken.'" "*" "'.escapeshellcmd(str_replace(",", " ", $skip)).'"', true);
    
                                }else{
                                                                                        
                                    $s = ssh2_exec($con, 'cd /srv/scripts; sudo ./backup_server.sh '.$core->framework->server->getData('ftp_user').' "'.$filename.' '.$core->framework->server->nodeData('node').' '.$core->framework->server->getData('hash').' '.$backupToken.'" "'.escapeshellcmd(str_replace(",", " ", $backup)).'" "'.escapeshellcmd(str_replace(",", " ", $skip)).'"', true);
                            ...
                            ssh2_exec($con, 'cd /srv/scripts; sudo ./send_command.sh '.$core->framework->server->getData('ftp_user').' "save-on"', true);
