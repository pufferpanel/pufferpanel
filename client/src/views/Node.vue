<template>
  <v-row>
    <v-col cols="12" md="6" offset-md="3">
      <v-alert v-if="updateSuccess === true" type="success" v-text="$t('common.NodeUpdateSuccess')" />
      <v-alert v-if="updateSuccess === false" type="error" v-text="$t('common.NodeUpdateError')" />
      <v-card>
...
export default {
  ...
  methods: {
    ...
    updateNode () {
      const ctx = this
      ctx.$http.post(`/api/nodes/${ctx.$route.params.id}`, ctx.node).then(function (response) {
