...
func (op MojangDl) Run(args pufferpanel.RunOperatorArgs) pufferpanel.OperationResult {
	...
	for _, version := range data.Versions {
		if version.Id == targetVersion {
			...
			err = downloadServerFromJson(version.Url, args.Server.GetFileServer(), op.Target, env)
			return pufferpanel.OperationResult{Error: err}
		}
	}
...
func downloadServerFromJson(url string, fs files.FileServer, target string, env *pufferpanel.Environment) error {
	...
	return pufferpanel.DownloadFile(serverBlock.Url, fs, target, env)
}
