...
if(!isset($_POST['pack_name'], $_POST['pack_hash'], $_POST['pack_version'], $_POST['pack_minram'], $_POST['pack_permgen']))
	Page\components::redirect('../../edit.php?mid='.$_POST['pack_hash'].'&disp=missing_args');

/*
 * Validate Modpack Name
 */
if(!preg_match('/^[\s\w.()-]{1,64}$/', $_POST['pack_name']))
	Page\components::redirect('../../edit.php?mid='.$_POST['pack_hash'].'&error=pack_name&disp=pn_fail');

/*
 * Validate Modpack Jar Name
 */
//if(!preg_match('/^[\s\w.-]{1,64}$/', $_POST['server_jar']))
//	Page\components::redirect('../../edit.php?mid='.$_POST['pack_hash'].'&error=server_jar&disp=pn_fail');
...
if(!is_numeric($_POST['pack_minram']) || !is_numeric($_POST['pack_permgen']))
	Page\components::redirect('../../edit.php?mid='.$_POST['pack_hash'].'&error=pack_minram|pack_permgen&disp=num_fail');

/*
 * Validate Version
 */	
if(!preg_match('/^[\w.-]{1,64}$/', $_POST['pack_version']))
	Page\components::redirect('../../edit.php?mid='.$_POST['pack_hash'].'&error=pack_version&disp=ver_fail');
...
//Redirect
Page\components::redirect('../../edit.php?mid='.$_POST['pack_hash'].'&success=true');
