...
if($core->auth->isLoggedIn($_SERVER['REMOTE_ADDR'], $core->auth->getCookie('pp_auth_token')) === true)
	Page\components::redirect('core/servers');

if(is_null($route->params['do']) || $route->params['do'] != "do.login")
	echo $twig->render(
			'panel/index.html', array(
				'error' => $route->params['do'],
				'footer' => array(
...
if(!is_null($route->params['do']) && $route->params['do'] == 'do.login'){
	...
	if($core->auth->verifyPassword($_POST['email'], $_POST['password']) === true){
		...
		Page\components::redirect('core/index/do.error');
