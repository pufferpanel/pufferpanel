...
var Routes = {
    post: {
        login: function (request, reply) {
            ...
            Authentication.loginUser(request.payload.email, request.payload.password, request.payload.totpToken, request.info.remoteAddress, function (err, data) {
    ...
    get: {
        loginAdmin: function (request, reply) {

            if (!request.state.pp_session || !request.state.pp_session.id) {
                return reply.redirect('/auth/login').code(403);
            }

            // Verify that Session is Valid for User
            var userSession = request.state.pp_session;

            UserController.getData(userSession.id, function (err, user) {

                if (user.sessionToken !== userSession.sessionToken || user.sessionIp !== userSession.sessionIp) {
                    return reply.redirect('/').code(403);
                }
