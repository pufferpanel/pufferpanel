...
var Routes = {
    post: {
        login: function (request, reply) {
            ...
            Authentication.loginUser(request.payload.email, request.payload.password, request.payload.totp_token, function (err, successful, data) {

                if (err !== undefined || !successful) {
                    ...
                    if (typeof data.session.language === undefined) {
                        data.session.language = 'en';
                    }
                    reply.state('pp_language', data.session.language, {
                        ttl: 60 * 60 * 24 * 1000,
                        path: '/',
                        isSecure: false,
                        isHttpOnly: true
        ...
        totp: function (request, reply) {
            ...
            Authentication.isTOTPEnabled(request.payload.check, function (err, data) {
