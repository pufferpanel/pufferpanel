...
var SettingsController = Rfr('lib/api/controllers/settings.js');
var AuthenticationController = Rfr('lib/api/controllers/authentication.js');
var UserController = Rfr('lib/api/controllers/user.js');
var UserVisibleError = Rfr('lib/errors/UserVisibleError.js');

var Routes = {};

Routes.getValidatePasswordRequest = function (request, reply) {

    AuthenticationController.validatePasswordReset(request.params.token, function (err) {

        if (err && !(err instanceof UserVisibleError)) {
            Logger.error('An error occured while attempting to perform a password reset validation.', err);
            return reply(Boom.badImplementation());
        }

        if (!err) {
            request.session.flash('passwordSuccess', 'Your password has been reset and emailed to you.');
        } else {
