 ...
 */
var Authentication = requireFromRoot('lib/services/auth');

var Routes = {
  post: {
    login: function (request, reply) {

      Authentication.loginUser(request.payload.email, request.payload.password, request.payload.totp_token, request.payload.remoteAddress, function (success, data) {
        if (success) {
    ...
    totp: function (request, reply) {
      reply(Authentication.isTOTPEnabled(request.payload.check) ? 'true' : 'false');
    },
