...
UserController.updatePassword = function (id, oldPassword, newPassword, next) {
    ...
    // Validate Data
    Joi.validate({ np: newPassword, op: oldPassword }, schema, { abortEarly: false }, function (err, value) {

        if (err) {
            _.each(err.details, function (v, key) {
                errorResponse.push(v.message);
            });
            return next(null, errorResponse);
        }

        // Select the User and Check Data
        UserModels.select({ id: id }, function (err, user) {
            ...
            if (!Bcrypt.compareSync(oldPassword, user.password)) {
                return next(null, 'The provided password did not match the current account password.');
            }
