...
var Rfr = require('rfr');
var SettingsModel = Rfr('lib/api/models/settings.js');
var Joi = require('joi');
...
AdminSettingsController.updateCaptchaSettings = function (publicKey, secretKey, next) {

    var schema = {
        public: Joi.string().required().label('Public reCAPTCHA Key'),
        secret: Joi.string().required().label('Secret reCAPTCHA Key')
    };
    var errorResponse = [];

    Joi.validate({ public: publicKey, secret: secretKey }, schema, { abortEarly: false }, function (err) {
