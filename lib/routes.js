...
var CoreSettingsController = Rfr('lib/controllers/coresettings.js');
...
server.methods.locals('settings', function (setting) {

    CoreSettingsController.returnSetting(setting, function (err, result) {

        if (err) {

            Logger.error('An error occured in server.methods.locals.settings().', err);
            return 'error';
        }
