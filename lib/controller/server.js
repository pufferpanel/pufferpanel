...
/**
 * Creates a new ServerController backed by data from the given collection.
 * If no collection is passed, a default is used which is backed by RethinkDB
 *
 * @param {ServerCollection} collection - Data collection to use
 * @constructor
 */
var ServerController = function (collection) {
    this.collection = collection ? collection : new ServerCollection();
};
...
ServerController.prototype.create = function (name, ownerUUID, nodeUUID, plugin, ip, port, data) {
    ...
    var self = this;
    var server = new Server(Uuid.v4(), name, type, [{ ip: ip, port: port }], ownerUUID, nodeUUID, false, data);
    self.collection.add(server);
    return server;
};
 ...
 * @returns {Server[]} - Array of servers this user owns, will never be undefined
 */
ServerController.prototype.getForUser = function (userUUID) {
    Validate.isUUID(userUUID, 'userUUID');

    var self = this;
    var servers = self.collection.getByOwner(userUUID);
    return servers;
};
...
ServerController.prototype.updateName = function (uuid, name) {
    ...
    var self = this;
    self.collection.update(uuid, { name: name });
};
...
ServerController.prototype.updateOwner = function (serverUUID, userUUID) {
    ...
    var self = this;
    self.collection.update(serverUUID, { owner: userUUID });
};
...
ServerController.prototype.addAddress = function (serverUUID, ip, port, primary) {
    ...
    var self = this;
    var server = self.collection.get(serverUUID);
    if (server) {
        ...
        self.collection.update(server.getUUID(), addresses);
    } else {
...
ServerController.prototype.changePluginData = function (serverUUID, data) {
    ...
    self.collection.update(serverUUID, { data: data });
};
